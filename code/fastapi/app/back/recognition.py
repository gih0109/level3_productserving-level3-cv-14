...
def load_ocr_model(img_scale: tuple = (32, 100), # (height, width)
                   num_fiducial: int = 20,
                   input_channel: int = 1,
...
def text_recognition(model = None,
                     img_folder: str = None,
    ...
    AlignCollate_demo = AlignCollate(imgH=img_scale[0], imgW=img_scale[1], keep_ratio_with_pad=keep_ratio_with_pad)
    ocr_data = RawDataset(root=img_folder, img_scale=img_scale, rgb=rgb)  # use RawDataset
    ocr_data_loader = torch.utils.data.DataLoader(
        ocr_data, batch_size=batch_size,
        shuffle=False,
